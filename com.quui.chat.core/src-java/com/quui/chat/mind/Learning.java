...
public class Learning {
	...
	protected String learnNewTopics(String rawUserInput, Vector<String> tokens,
		...
		for (String name : tokens) {
			...
			mind.isWordNetEnabled && !WNLookup.isContentWord(name)
					|| Mind.topicsIndices.keySet().contains(name))
				continue;
	...
	private Topic createNewTopic(String rawUserInput, Synset synset) {
		...
		for (int i = 0; i < words.length; i++) {
			...
			if (GlobalProperties.getInstance().getLearnFromWN()) {
		...
		learnRelatedWordsAsKeys(topic, key, GlobalProperties.getInstance()
				.getLearnFromWN());
	...
	private void addToMap(Topic topic) {
		for (Iterator<String> iterator = topic.getKeys().iterator(); iterator.hasNext();) {
			String k = (String) iterator.next();
			if (!Mind.topicsIndices.keySet().contains(k)) {
				Vector<Integer> v = new Vector<Integer>();
				v.add(new Integer(Mind.topics.size()));
				Mind.topicsIndices.put(k, v);
			} else {
				Vector<Integer> topicIndicesForKey = Mind.topicsIndices.get(k);
				topicIndicesForKey.add(new Integer(Mind.topics.size()));
				Mind.topicsIndices.put(k, topicIndicesForKey);
			}
		}
	}
	 ...
	 * @param key
	 *            The basic key
	 * @param learnAsAnswersToo
	 */
	private void learnRelatedWordsAsKeys(Topic topic, String key,
			boolean learnAsAnswersToo) {
		if (mind.isWordNetEnabled) {
			// learnParentsAsKeys(topic, key);
			learnChildrenAsKeys(topic, key, learnAsAnswersToo);
		}
	}
